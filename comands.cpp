...
void info_command::help()
{
	...
	cout << "calc - \t\t включает режим калькулятор" << endl;
}
...
{
	cout << "Включен режим калькулятора" << endl;
	...
	while (true)
	{
		...
		{
			cout<<"Результат = "<<calc(expression)<<endl;
		}
	}
}

bool calculate_comand::check_correct(std::string& str)
{
	try
	{
...
bool calculate_comand::check_scobs(std::string& str)
{
...
double calculate_comand::math_operation(double value1, double value2, char opr)
{
	try//обработчик исключений
	{
		if (opr == '+')//сложение
...
bool calculate_comand::check_value(char symbol)
{
	try//обработчик исключений
	{
...
bool calculate_comand::check_operator(char symbol)
{
	try//обработчик исключений
	{
...
void calculate_comand::get_attribute(const std::string& str, std::string& value_str, int& condition, size_t& pozition)
{
	try//обработчик исключений
	{
		std::string value_s = "";
...
int calculate_comand::get_priority(char opr)
{
	// +, - : 1
	// *, / : 2
	// ( : 3
...
int calculate_comand::get_quantity_value(const std::string& str)
{
	try
	{
		std::string value_s = "";
...
int calculate_comand::get_quantity_operator(const std::string& str)
{
	try
	{
		int result = 0;
...
double calculate_comand::calc(std::string expression)
{
	try
	{
